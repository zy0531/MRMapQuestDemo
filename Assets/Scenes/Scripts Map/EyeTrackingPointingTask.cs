...
public class EyeTrackingPointingTask : MonoBehaviour
{
    [Header("Gaze data")]
    ...
    private static readonly string[] ColumnNames = { "Trial", "Time",
        ...
        "ReferenceLandmark", "DisplayLandmark",
        "GroundTruthDirection", "GazeAngleDeviation"
        };
    ...
    //add
    [SerializeField] TeleportInPointingTask task;
    [SerializeField] ControllerTriggerFunc controllerTriggerFunc;
    //add
    ...
    void GetDevice()
    {
        ...
        Debug.Log(device + "GetDevice Done!");
    }
    ...
    {
        string[] logData = new string[36];

        // * Trial *
        logData[0] = controllerTriggerFunc.TrialNum.ToString();
        ...
        // ref landmark & display landmark
        var referencelandmark = task.referenceLandmark_name;
        var displaylandmark = task.displayLandmark_name;
        logData[32] = referencelandmark;
        logData[33] = displaylandmark;

        // Calculate Angle between "gazeVector" (i.e., direction) and "groundtruthDirection"
        Vector3 groundtruthDirection = task.GroundTruthDirection;
        groundtruthDirection = Vector3.ProjectOnPlane(groundtruthDirection, new Vector3(0, 1, 0));//xz plane
        logData[34] = groundtruthDirection.ToString("F3");
        logData[35] = invalid ? "" : Vector3.Angle(direction, groundtruthDirection).ToString("F3");
